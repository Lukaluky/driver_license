...
public sealed class MvdExternalCheckProvider : IExternalCheckProvider
{
    private readonly HttpClient _httpClient;
    private readonly string _wiremockBaseUrl;
    public string Name => "MVD";

    public MvdExternalCheckProvider(HttpClient httpClient, IConfiguration config)
    {
    ...
    {
        var response = await _httpClient.GetAsync($"{_wiremockBaseUrl}/api/mvd/check?iin={iin}", cancellationToken);
